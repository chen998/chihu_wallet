...
public class MemberCollectionController {
    ...
    @ResponseBody
    public CommonResult deleteProduct(@RequestBody MemberProductCollection productCollection) {
        int count = memberCollectionService.deleteProduct(productCollection.getMemberId(), productCollection.getProductId());
        if (count > 0) {
            return CommonResult.success(count);
        } else {
            return CommonResult.failed();
